...
export default function ServiceCreateForm(props) {
  ...
  const [doctor, setDoctor] = React.useState(initialValues.doctor);
  const [like, setLike] = React.useState(initialValues.like);
  const [errors, setErrors] = React.useState({});
  const resetStateValues = () => {
    setName(initialValues.name);
    setDescription(initialValues.description);
    setDoctor(initialValues.doctor);
      ...
      <TextField
        ...
        onChange={(e) => {
          ...
          if (onChange) {
            ...
            const result = onChange(modelFields);
            value = result?.doctor ?? value;
          }
          if (errors.doctor?.hasError) {
            runValidationTasks("doctor", value);
          }
          setDoctor(value);
        }}
        onBlur={() => runValidationTasks("doctor", doctor)}
        errorMessage={errors.doctor?.errorMessage}
        hasError={errors.doctor?.hasError}
        {...getOverrideProps(overrides, "doctor")}
      ></TextField>
      <TextField
        label="Like"
        isRequired={true}
